...
function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-transparent py-6  sm:px-6">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-[48px] font-bold text-white mb-2 leading-tight">
                {`Welcome back, ${user?.name?.split(" ")[0] || "Learner"} 👋`.split("").map((char, i) => (
                  <span
                    key={i}
        ...
        {stats && (
          <>
            {/* Stats Grid */}
            <div className="mb-10">
              <ButtonHeading text="OVERVIEW" className="border-none pl-0 pb-0"/>
              <div className="grid md:grid-cols-4 gap-4">
                <div className="bg-[#0f0f0f] border border-[#1f1f1f] hover:border-violet-700 p-5">
                ...
                <div className="bg-[#0f0f0f] border border-[#1f1f1f] hover:border-violet-700 p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center">
                      <Bookmark size={20} className="text-violet-700" />
                    </div>
                ...
                <div className="bg-[#0f0f0f] border border-[#1f1f1f] hover:border-violet-700 p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center">
                      <Flame size={20} className="text-violet-700" />
                    </div>
                    <span className="text-[12px] text-[#555] uppercase tracking-wide">Current Streak</span>
                  </div>
                  <div className="text-[36px] font-bold text-white">{stats.streak_count || 0} <span className="text-[16px] text-[#666]">days</span></div>
                </div>
              </div>
            </div>
            ...
            {/* Activity Section */}
            <div className="mb-10">
              <ButtonHeading text="LEARNING_ACTIVITY" className="border-none pl-0 pb-0"/>
              <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-[#0f0f0f] border border-[#1f1f1f] hover:border-violet-700 p-5">
                  <div className="text-[11px] text-violet-700 font-bold uppercase tracking-wide mb-2 font-mono">AI Summaries Viewed</div>
                  <div className="text-[28px] font-bold text-white">{stats.posts_watched || 0}</div>
                </div>
                
                <div className="bg-[#0f0f0f] border border-[#1f1f1f] hover:border-violet-700 p-5">
                  <div className="text-[11px] text-violet-700 font-bold uppercase tracking-wide mb-2 font-mono">Topics Created</div>
                  <div className="text-[28px] font-bold text-white">{stats.subjects_created_count || 0}</div>
                </div>
                
                <div className="bg-[#0f0f0f] border border-[#1f1f1f] hover:border-violet-700 p-5">
                  <div className="text-[11px] text-violet-700 font-bold uppercase tracking-wide mb-2 font-mono">Total Learning Time</div>
            ...
            {stats.subjects_progress && stats.subjects_progress.length > 0 && (
              <div className="mb-8">
                <div className="flex items-center justify-between  px-1">
                  <ButtonHeading text="LEARNING_MONITOR" className="border-none pl-0 pb-0"/>
                  <div className="text-[12px] font-bold text-[#444] font-mono text-right">
                ...
                <div className="bg-[#0a0a0a] border border-[#222] p-8 relative overflow-hidden">
                  ...
                  <div className="relative z-10 space-y-8">
                    {stats.subjects_progress.map((subject, idx) => (
                      <div key={idx}>
                        <div className="flex items-end justify-between mb-2">
                          <span className="text-[13px] font-bold text-[#888] font-mono uppercase tracking-wider">{subject.name.toUpperCase().replace(/\s+/g, '_')}</span>
                          <span className="text-[13px] text-violet-700 font-bold font-mono">{String(subject.progress || 0).padStart(2, '0')}</span>
                        </div>
                        ...
                        <div className="h-6 w-full bg-[#111] border border-[#222]">
                          ...
                          >
                            <div className="absolute inset-0 bg-violet-700" style={{
                    ...
                    <div className="pt-8 mt-4 border-t border-[#222] grid grid-cols-2 md:grid-cols-4 gap-8">
                      <div>
                        <div className="text-[24px] font-bold text-violet-700 mb-1">
                           {stats.subjects_progress.reduce((acc, curr) => acc + (curr.progress || 0), 0) / stats.subjects_progress.length | 0}%
                        </div>
                        <div className="text-[13px] text-[#555] font-mono tracking-widest uppercase">AVG_MASTERY</div>
                      </div>
                      <div>
                        <div className="text-[24px] font-bold text-violet-700 mb-1">
                          {stats.streak_count || 1}x
                        </div>
                        <div className="text-[13px] text-[#555] font-mono tracking-widest uppercase">CONSISTENCY</div>
                      </div>
                      <div>
                        <div className="text-[24px] font-bold text-violet-700 mb-1">
                          100%
                        </div>
                        <div className="text-[13px] text-[#555] font-mono tracking-widest uppercase">ACCURACY</div>
                      </div>
                      <div>
                        <div className="text-[18px] font-bold text-violet-700  mt-2 mb-1">
                          SOTA
                        </div>
                        <div className="text-[13px] text-[#555] font-mono tracking-widest uppercase">PERFORMANCE</div>
                      </div>
                    </div>
                  </div>
                </div>
